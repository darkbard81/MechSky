...
let scenario: DevBattleScenario | null = null;
let scenarioError: unknown = null;

try {
  scenario = resolveDevBattleScenario(window.location);
} catch (error: unknown) {
  scenarioError = error;
}

const game = new GameApp(elements, scenario);

if (scenarioError !== null) {
  console.error(scenarioError);
  game.reportBootFailure(scenarioError);
} else {
